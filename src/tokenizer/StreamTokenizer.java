    ...
    public int nextToken() throws IOException {
        ...
        while ((currentType & TOKEN_WHITE) != 0) {
            ...
            if (currentChar == '\r') {
                ...
                // commented out by Mikal Ziane to keep layout
            	//currentChar = read();
            	peekChar =-2;					// added by Mikal Ziane
            	return (ttype = currentChar);	// added by Mikal Ziane
            }
            if (currentChar == -1) {
                return (ttype = TT_EOF);
            }
        ...
        if ((currentType & TOKEN_DIGIT) != 0) {
            ...
            // added by Mikal Ziane
            if (haveDecimal && digits.length() == 1) {
                // Didn't get any other digits other than '.'
                return (ttype = '.');
            }
            try {
